...
const MyEnrollments = () => {
  ...
  const getCourseProgress = async()=>{
    try {
      ...
      const tempProgressArray = Promise.all(
          ...
          <tbody className='text-gray-700 '>
            {enrolledCourses.map((course, index) => (
              <tr key={index} className='border-b border-gray-500/20'>
                ...
                <td className='px-4 py-3 max-sm:hidden'>
                  <button onClick={()=>navigate('/player/'+ course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 msx-sm:text-xs text-white'>{progressArray[index] && progressArray[index].lectureCompleted/progressArray[index].totalLectures ===1 ? "Completed" : "On Going"} </button>
                </td>
              </tr>
            ))}
          </tbody>
